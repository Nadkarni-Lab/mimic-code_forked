...
WITH tm AS (
    ...
    FROM `physionet-data.mimiciii_clinical.liftovermimiciv_ventilator_setting`
    UNION DISTINCT
    SELECT icustay_id, charttime
    FROM `physionet-data.mimiciii_clinical.liftovermimiciv_oxygen_delivery`
)

, vs AS (
    SELECT tm.icustay_id, tm.charttime
    ...
    FROM tm
    LEFT JOIN `physionet-data.mimiciii_clinical.liftovermimiciv_ventilator_setting` vs
        ON tm.icustay_id = vs.icustay_id
            AND tm.charttime = vs.charttime
    LEFT JOIN `physionet-data.mimiciii_clinical.liftovermimiciv_oxygen_delivery` od
        ON tm.icustay_id = od.icustay_id
...
, vd0 AS (
    SELECT
        ...
        , LAG(
            ...
            PARTITION BY icustay_id, ventilation_status ORDER BY charttime
        ) AS charttime_lag
...
, vd2 AS (
    SELECT vd1.icustay_id, vd1.charttime
